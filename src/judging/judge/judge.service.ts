...
export class JudgeService {
  ...
  async rankSubmissions(
    ...
    const submissions = rankings.map((submissionId) =>
      judge.group.submissions.find(({ id }) => id === submissionId),
    );
  ...
  async submitRanking(userId: string): Promise<void> {
    ...
    const rankings = judge.rankings.map((submissionId) =>
      judge.group.submissions.find(({ id }) => id === submissionId),
    );
    ...
    if (rankings.length > 5) {
      throw new HttpException(
        `Please only rank your top 5`,
        HttpStatus.BAD_REQUEST,
      );
    }
    if (
      rankings.length < 5 &&
      rankings.length !== judge.group.submissions.length
    ) {
      throw new HttpException(
        `Please rank at-least 5 projects`,
        HttpStatus.BAD_REQUEST,
      );
    }
