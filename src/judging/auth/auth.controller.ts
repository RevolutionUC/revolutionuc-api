...
export class JudgingAuthController {
  ...
  login(
    ...
    return this.authService.login(credentials.username, credentials.password, judgingRoles);
  }

  @Get(`me`)
  @UseGuards(AuthGuard(`jwt`), RoleGuard)
  async getUserDetails(@CurrentUser() { id }: CurrentUserDto): Promise<Pick<User, 'id' | 'username' | 'role'>> {
    console.log({ id });
