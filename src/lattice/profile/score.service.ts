...
export class ScoreService {
  ...
  private scoreProfileBothWays(from: ProfileDTO, to: ProfileDTO): ScoredProfileDTO {
    const score = this.scoreProfileOneWay(from, to) + this.scoreProfileOneWay(to, from);
    return { ...to, score };
  }

  private scoreProfiles(from: ProfileDTO, profiles: Array<ProfileDTO>): Array<ScoredProfileDTO> {
